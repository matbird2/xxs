...
import com.xxs.leon.xxs.adapter.base.BaseListAdapter;
import com.xxs.leon.xxs.adapter.base.ViewHolder;
import com.xxs.leon.xxs.listener.NewRecordPlayClickListener;
import com.xxs.leon.xxs.rest.engine.CommenEngine;
import com.xxs.leon.xxs.rest.engine.impl.CommenEngineImpl;
import com.xxs.leon.xxs.rest.engine.impl.CommenEngineImpl_;
...
public class MessageChatAdapter extends BaseListAdapter<BmobMsg>{
    ...
    public View bindView(int position, View convertView, ViewGroup parent) {
        ...
        if(avatar != null && !avatar.equals("")){
            Glide.with(mContext).load(avatar).placeholder(R.drawable.default_head_photo).error(R.drawable.default_head_photo).bitmapTransform(new CropCircleTransformation(mContext)).crossFade(500).centerCrop().into(iv_avatar);
        }else{
            iv_avatar.setImageResource(R.drawable.default_head_photo);
        }
        ...
        tv_time.setText(TimeTool.getChatTime(Long.parseLong(item.getMsgTime())));
...
//			如果是发送的图片的话，因为开始发送存储的地址是本地地址，发送成功之后存储的是本地地址+"&"+网络地址，因此需要判断下
            ...
            Glide.with(mContext).load(showUrl).placeholder(R.drawable.default_image_loading).error(R.drawable.default_loading_error).bitmapTransform(new CropCircleTransformation(mContext)).crossFade(500).centerCrop().into(iv_picture);
        }else{
            Glide.with(mContext).load(text).placeholder(R.drawable.default_image_loading).error(R.drawable.default_loading_error).bitmapTransform(new CropCircleTransformation(mContext)).crossFade(500).centerCrop().into(iv_picture);
        }
    }
